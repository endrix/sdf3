 ...
 *
 * $Id: static_periodic_scheduler.cc,v 1.1.2.4 2010-04-25 02:08:52 mgeilen Exp $
 *
...
#define PRODUCE(c,n)        CH(c) = CH(c) + n;
                                  ...
                                  const long long int periodicity)
    {
        ...
        {
            firingIdx[i] = 0;
            startTime[i].resize((size_t)(repVec[i] * periodicity));
        ...
        while (true)
        {
                    ...
                    if (outputActor == a->getId())
                    {
                        ...
                        if (repCnt == outputActorRepCnt)
                        {
                            ...
                            if (!storeState(currentState, recurrentState))
                            {
                                ...
                                long long int minStartTime = CSIZE_MAX;
                 ...
                 iter != g->actorsEnd(); iter++)
            {
                ...
                while (startTime[a->getId()][(size_t) firingIdx[a->getId()]] + period
                       <= globalTime && actorReadyToFire(a))
                {
                    // Fire actor a
                    startActorFiring(a);
                    startTime[a->getId()][(size_t) firingIdx[a->getId()]] = globalTime;
                    iterCnt[a->getId()][(size_t) firingIdx[a->getId()]]++;
            ...
            for (uint a = 0; a < g->nrActors(); a++)
            {
                ...
                if (startTime[a][(size_t) firingIdx[a]] + period - globalTime > 0)
                {
                    actClk = (SDFtime)(startTime[a][(size_t) firingIdx[a]] + period - globalTime);
                    if (clkStep > actClk)
                        clkStep = actClk;
                }
            }
        ...
        const long long int period)
    {
        ...
        while (true)
        {
                 ...
                 iter != g->actorsEnd(); iter++)
            {
                ...
                while (startTime[a->getId()][(size_t) firingIdx[a->getId()]]
                       + period * iterCnt[a->getId()][(size_t) firingIdx[a->getId()]]
                       <= globalTime)
                {
                    // Ready to fire actor a?
            ...
            for (uint a = 0; a < g->nrActors(); a++)
            {
                ...
                // Time till next point from which actor is allowed to fire
                if (startTime[a][(size_t) firingIdx[a]] + period * iterCnt[a][(size_t) firingIdx[a]]
                    - globalTime > 0)
                {
                    actClk = (SDFtime)(startTime[a][(size_t) firingIdx[a]]
                                       + period * iterCnt[a][(size_t) firingIdx[a]] - globalTime);
                    if (clkStep > actClk)
                        clkStep = actClk;
                }
            }
